...
module.exports = async(param) => {
  ...
  const _crawl = async(callback) => {
    ...
    if (currentUrl) {
      ...
      try {
        ...
        await page.waitFor(param.speed);
        ...
        console.log(chalk.red('Catch Error: ' + currentUrl));
      ...
      if (param.screenshots) {
        ...
        await page.waitFor(param.screenshotsSpeed);
  ...
  await _crawl(() => {
    ...
    for (let i = 0; i < urlListLength; i++) {
      ...
      // index.html index.htm index.php を削除
      if (path.slice(-10) === 'index.html') {
        path = path.slice(0, -10);
      } else if (path.slice(-9) === 'index.htm' || path.slice(-9) === 'index.php') {
        path = path.slice(0, -9);
      }
